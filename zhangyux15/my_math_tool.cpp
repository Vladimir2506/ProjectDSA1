...
void calc_sp(Mat &matImgOrigin, Mat &output_sp_label, Mat &output_sp_contour, int &output_sp_num)
{
	...
	Mat frame, labels, matMask;
	frame = matImgOrigin.clone();				// 拷贝一份原图
	...
	/*
	frame.setTo(Scalar(255, 255, 255), matMask);
...
void contour_to_set(vector<Point>& vecContours, vector<Point>& points_set)
{
	approxPolyDP(vecContours, vecContours, 0, true);
	points_set.resize(0);
	CvRect rect_bound = boundingRect(vecContours);
	...
	{
		copy_contour.push_back(vecContours[i] - zero_point);
	}
