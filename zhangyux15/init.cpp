...
void read_view_data(ImgData& imgdata, string path_view, bool with_file)
{
	...
	// 得到深度图
	Mat matDepth(HEIGHT, WIDTH, CV_32F, depth_vec);
	// 读取原图
	Mat matImgOrigin = imread(path_view + "\\undist-L2.png");
	// 得到超像素
	Mat sp_label, sp_contour; int sp_num;
	if (!with_file)
	{
		calc_sp(matImgOrigin, sp_label, sp_contour, sp_num);		//重新计算超像素存入文件
	...
	// 生成imgdata
	imgdata = ImgData(id, cam, matImgOrigin, matDepth, sp_label, sp_contour, sp_num);
}
